...
I'm testing to see if being fast lands me a job.
...
1. **Detect** — a Cloudflare Worker polls every board hourly (rotating shard schedule), with two independent fallback triggers (GitHub Actions + cron-job.org) so a dead cron never means missed jobs. Relevance scoring keeps the phone quiet for anything outside your scope.
